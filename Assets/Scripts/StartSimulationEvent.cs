...
public class StartSimulationEvent : MonoBehaviour
{
    ...
    int healerToActivateBench;
    int tankToActivateBench;
    ...
    void CalculateDpsAfterSurvi()
    {
        ...
        bool checkTanksAndHealers = true;
        ...
        for (int retryDead = 0; retryDead <= tryNumber; retryDead++)
        {
            ...
            while ((encounterTime <= bossData.berserkInSeconds) && (bossActuallHp > 0))
            {
                if (((AreTanksAlive() == false) || (AreHealersAlive() == false)) && (retryDead == tryNumber) && (checkTanksAndHealers == true))
                {
                    Debug.Log("Triggered tank/healer event");
                    benchTime = encounterTime;
                    ...
                    if (AreTanksAlive() == false)
                    {
                        combatLog[combatLogIndex] = "All tanks died";
                    }
                    else if (AreHealersAlive() == false)
                    {
                        combatLog[combatLogIndex] = "All healers died";
                    }
                    combatLogIndex++;

                    checkTanksAndHealers = false;
                }
                ...
                for (int index = 0; index < 20; index++)
                {
                    ...
                    if ((raidRooster[index] != null))
                    {
                        ...
                        if ((isAlive[index] == false) && (deadcheck[index] == true) && (encounterTime >= deathTime[index]) && (ressTime[index] == 0) && (benchdeathTime[index] == 0))
                        {
                            ...
                            {
                                if ((resurrectionCharges > 0) && (ressTime[index] == 0))
                                {
...
/*                                else if ((raidRooster[index].GetPlayerClass() == "Shaman") && (ressTime[index] == 0))
                ...
                if (((Affix[8] == true) && (isBenchActive() == true) && (retryDead == tryNumber) && (intBenchTime <= encounterTime)) || (((AreTanksAlive() == false) || (AreHealersAlive() == false)) && (retryDead == tryNumber) && (intBenchTime <= encounterTime)))
                {
                    for (int index = 0; index < 20; index++)
                    {
                        if ((((randomSurvi[index] < benchRandom) && (isAlive[index] == true)) && (benchDead[index] == false)) || ((deathTime[index] >= (benchTime + 5)) && (isAlive[index] == false) && (benchDead[index] == false)) || ((isAlive[index] == false) && (deadcheck[index] == true) && (benchDead[index] == false) && ((ressTime[index] != 0) && (ressTime[index] <= benchTime))))
                        {
                            ...
                            if ((deadcheck[index] == false) || (((deadcheck[index] == true) && (ressTime[index] < benchTime))))
                            {
                                ...
                                if (AreTanksAlive() == false)
                                {
                                    combatLog[combatLogIndex] = "Player " + raidRooster[index].GetPlayerName() + " died at " + (intDeathTime / 60).ToString() + ":" + (intDeathTime % 60).ToString("00") + ". Aggro.";
                                }
                                else if (AreHealersAlive() == false)
                                {
                                    combatLog[combatLogIndex] = "Player " + raidRooster[index].GetPlayerName() + " died at " + (intDeathTime / 60).ToString() + ":" + (intDeathTime % 60).ToString("00") + ". Lack of Healing.";
                                }
                                else
                                {
    ...
    void CalculateSurvi()
    {
        ...
        if ((Affix[8] == true) && (isBenchActive() == true))
        {
            for (int index = 0; index < 20; index++)
            {
                if (raidRooster[index].GetPlayerName() == "Muminoses")
                {
                    if (isAlive[index] == true)
                    {
...
/*                                else if ((raidRooster[index].GetPlayerClass() == "Shaman") && (ressTime[index] == 0))
    ...
    bool AreTanksAlive()
    {
        ...
        for (int index = 0; index < 20; index++)
        {
            if(raidRooster[index].GetPlayerRole() == "Tank")
            {
                if((isAlive[index] == true) || ((ressTime[index] <= encounterTime) && (ressTime[index] != 0)) || (encounterTime <= deathTime[index]))
                {
                    status = true;
                    break;
                }
    ...
    bool AreHealersAlive()
    {
        bool status = false;

        for (int index = 0; index < 20; index++)
        {
            if (raidRooster[index].GetPlayerRole() == "Healer")
            {
                if ((isAlive[index] == true) || ((ressTime[index] <= encounterTime) && (ressTime[index] != 0)) || (encounterTime <= deathTime[index]))
                {
                    status = true;
